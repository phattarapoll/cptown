document.addEventListener('DOMContentLoaded', function() {
        ...
        try {
            ...
            if (data.success) {
                ...
                data.officers.forEach(off => {
                    ...
                    card.innerHTML = `<i class="fa-solid fa-user-doctor" style="font-size: 1.5rem; margin-bottom: 8px; display: block;"></i><h4 style="margin:0;">${off.name}</h4>`;
                    card.onclick = () => {
    ...
    function createKeypad() {
        ...
        const clearBtn = document.createElement('button'); clearBtn.className = 'keypad-btn'; clearBtn.textContent = 'C';
        ...
        const delBtn = document.createElement('button'); delBtn.className = 'keypad-btn'; delBtn.innerHTML = '<i class="fa-solid fa-delete-left"></i>';
            ...
            if (currentPassword.length === 4) {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        document.getElementById('floating-history-button').classList.remove('hidden');
    ...
    function renderHistory(history) {
        leaveHistoryTableBody.innerHTML = history.length > 0 ? history.map(l => `
            <tr>
                ...
                <td><button class="btn-cancel" data-id="${l.leaveId}"><i class="fa-solid fa-trash"></i></button></td>
            </tr>
...
function handleCancel(id) { 
    if(confirm('ต้องการยกเลิกใบลา?')) {
        alert('ส่งคำขอยกเลิกแล้ว (ฟังก์ชันนี้ต้องเชื่อมต่อกับ API ยกเลิก)'); 
    }
}